...
sym_sym_t *sym_db = NULL;			//symbol database in no particular order
int sym_nsyms = 0;				//all symbol sorted by address
sym_sym_t **sym_syms = NULL;			//all symbol sorted by address
sym_sym_t **sym_syms_by_name = NULL;		//all symbols sorted by name
int sym_ntextsyms = 0;				//text symbols sorted by address
sym_sym_t **sym_textsyms = NULL;		//text symbols sorted by address
sym_sym_t **sym_textsyms_by_name = NULL;	//text symbols sorted by name
int sym_ndatasyms = 0;				//data symbols sorted by address
sym_sym_t **sym_datasyms = NULL;		//data symbols sorted by address
sym_sym_t **sym_datasyms_by_name = NULL;	//data symbols sorted by name
int syms_loaded = FALSE;			//symbols loaded?

#ifdef PRINT_SYMS
//convert BFD symbols flags to a printable string
//Returns the symbol flag string
char * flags2str(unsigned int flags)		// bfd symbol flags
{
	static char buf[256];
	char *p;
...
int acmp(sym_sym_t **sym1, sym_sym_t **sym2)
{
	return (int)((*sym1)->addr - (*sym2)->addr);
}

//qsort helper function
int ncmp(sym_sym_t **sym1, sym_sym_t **sym2)
{
	return strcmp((*sym1)->name.c_str(), (*sym2)->name.c_str());
}
...
sym_sym_t::sym_sym_t()
: name(), seg(), initialized(0), pub(0), local(0), addr(0), size(0)
{
}

/* load symbols out of FNAME */
void sym_loadsyms(const char *fname,	/* file name containing symbols */
	...
	mem_t* my_mem)
{
	...
	if((bfd_get_file_flags(abfd) & (HAS_SYMS|HAS_LOCALS)))
	{
		...
		for(i=0; i < nsyms; i++)
		{
				...
				&& RELEVANT_SCOPE(sym))
			{
				...
				sym_ndatasyms++;
#ifdef PRINT_SYMS
				fprintf(stderr,	"+sym: %s  sect: %s  flags: %s  value: 0x%08lx\n",
		...
		/* allocate symbol space */
			sym_db = new sym_sym_t[sym_nsyms];
		...
		for(debug_cnt=0, i=0; i < nsyms; i++)
		{
				...
				&& RELEVANT_SCOPE(sym))
			{
				...
				sym_db[debug_cnt].name = sym->name;
	...
	if(fread(&symhdr, sizeof(ecoff_symhdr_t), 1, fobj) < 1)
	...
	size_t fread_result = fread(strtab, len, 1, fobj);
	if(fread_result != 1)
	...
	sym_db = new sym_sym_t[len];

	/* allocate space for the external symbol entries */
	extr = (ecoff_EXTR *)calloc(symhdr.iextMax, sizeof(ecoff_EXTR));
	if(!extr)
		fatal("out of virtual memory");

	fseek(fobj, (long)symhdr.cbExtOffset, 0);
	fread_result = fread(extr, sizeof(ecoff_EXTR), symhdr.iextMax, fobj);
	if(fread_result != (size_t)symhdr.iextMax)
	...
	for(i=0; i < symhdr.iextMax; i++)
	{
		...
		case ECOFF_stStatic:
			/* from data segment */
			sym_db[sym_nsyms].name = &strtab[str_offset];
	...
	sym_syms = new sym_sym_t*[sym_nsyms];

	sym_syms_by_name = new sym_sym_t*[sym_nsyms];
	...
	/* sort by address */
	qsort(sym_syms, sym_nsyms, sizeof(sym_sym_t *), (int (*)(const void*, const void*))acmp);

	/* sort by name */
	qsort(sym_syms_by_name, sym_nsyms, sizeof(sym_sym_t *), (int (*)(const void*, const void*))ncmp);

	/* text segment sorted by address and name */
	sym_textsyms = new sym_sym_t*[sym_ntextsyms];

	sym_textsyms_by_name = new sym_sym_t*[sym_ntextsyms];
	...
	/* sort by address */
	qsort(sym_textsyms, sym_ntextsyms, sizeof(sym_sym_t *), (int (*)(const void*, const void*))acmp);

	/* sort by name */
	qsort(sym_textsyms_by_name, sym_ntextsyms, sizeof(sym_sym_t *), (int (*)(const void*, const void*))ncmp);

	/* data segment sorted by address and name */
	sym_datasyms = new sym_sym_t*[sym_ndatasyms];

	sym_datasyms_by_name = new sym_sym_t*[sym_ndatasyms];
	...
	/* sort by address */
	qsort(sym_datasyms, sym_ndatasyms, sizeof(sym_sym_t *), (int (*)(const void*, const void*))acmp);

	/* sort by name */
	qsort(sym_datasyms_by_name, sym_ndatasyms, sizeof(sym_sym_t *), (int (*)(const void*, const void*))ncmp);

	/* compute symbol sizes */
	for(i=0; i<sym_ntextsyms; i++)
	{
		sym_textsyms[i]->size =	(i != (sym_ntextsyms - 1)
	...
	{
		sym_datasyms[i]->size =	(i != (sym_ndatasyms - 1)
...
void sym_dumpsym(sym_sym_t *sym,	/* symbol to display */
	FILE *fd)			/* output stream */
{
	myfprintf(fd, "sym `%s': %s seg, init-%s, pub-%s, local-%s, addr=0x%08p, size=%d\n",
		sym->name.c_str(),
...
//bind address ADDR to a symbol in symbol database DB, the address must match exactly if
//EXACT is non-zero, the index of the symbol in the requested symbol database is returned
//in *PINDEX if the pointer is non-NULL
sym_sym_t *			/* symbol found, or NULL */
sym_bind_addr(md_addr_t addr,	/* address of symbol to locate */
	...
	sym_db_t db)		/* symbol database to search */
{
	...
	sym_db_t db)			/* symbol database to search */
{
	...
	high = nsyms-1;
	pos = (low + high) >> 1;
	while(!(/* exact string match */!(cmp = strcmp(syms[pos]->name.c_str(), name))))
	{
		if(cmp > 0)
			high = pos - 1;
		else
