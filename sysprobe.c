...
int fast_SRA()
{
	...
	{
		//signed int does SRL - emulate SRA
		return FALSE;
	}
}
...
#ifndef __GNUC__
		//probe compiler approach needed to concatenate symbols in CPP, new style concatenation is always used with GNU GCC
		{
			int i = 5, j;
			j = CAT(-,-i);
			if(j == 4)
			...
			{
				//old style symbol concatenation does not work, assume that new style symbol concatenation works
			}
			else
			{
				//huh!?!?!
...
#ifndef GZIP_PATH
		{
			for(int i=0; gzip_paths[i] != NULL; i++)
			{
