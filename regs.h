...
class regs_t {
	public:
		regs_t();
		regs_t(const regs_t & target);
		const regs_t & operator=(const regs_t & target);

		std::vector<md_gpr_t> regs_R;		//(signed) integer register file
		std::vector<md_fpr_t> regs_F;		//floating point register file
